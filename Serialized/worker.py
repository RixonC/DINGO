...
class Worker(object):
    ...
    def autoencoder(self, w):
        ...
        return torch.nn.Sequential(*modules).type(torch.Tensor) # the Autoencoder function


    def loss(self, w, AIDE_tau=0, AIDE_y=0):
        '''Return the loss at point w.
        Args:
            w: The current point.
            AIDE_tau: The parameter tau in the AIDE algorithm.
            AIDE_y: The current vector y in the AIDE algorithm.
        '''
        ...
        if self.config['obj_fun'] == 'Autoencoder':
            ...
            loss = reg + (1/self.s)*(self.s*self.d)*criterion(X, self.X).data # []
        ...
        if self.config['obj_fun'] == 'GMM':
            ...
            if alpha == 1:
                loss = reg - (1/self.s)*torch.sum(torch.log(c1)+a)
            elif alpha == 0:
                loss = reg - (1/self.s)*torch.sum(torch.log(c2)+b)
            else:
                m = torch.max(a, b) # [s]
                loss = reg - (1/self.s)*torch.sum(m + torch.log(c1*alpha*torch.exp(a-m) + c2*(1-alpha)*torch.exp(b-m)))
        ...
        if self.config['obj_fun'] == 'softmax':
            ...
            loss = reg + (1/self.s)*(torch.sum(log_sum_exp_trick) - torch.sum(XW*self.Y))

        if AIDE_tau != 0:
            loss += 0.5 * AIDE_tau * torch.norm(w-AIDE_y).pow(2)

        return loss


    def grad(self, w, AIDE_tau=0, AIDE_y=0):
        ...
        if self.config['obj_fun'] == 'Autoencoder':
            ...
            loss.backward()
            grad = reg_grad + (1/self.s)*torch.cat([T.grad.reshape(-1,1) for T in autoencoder.parameters()]).data

        if self.config['obj_fun'] == 'GMM':
            t = w[0:1] # [1,1]
            alpha = (torch.tanh(t)+1)/2 # [1,1]
            alpha_d = 1/(2*torch.cosh(t)**2) # [1,1]
            w1 = w[1:self.d+1] # [d,1]
            w2 = w[self.d+1:]  # [d,1]
            C1 = self.config['GMM_C1'] # [d,d]
            C2 = self.config['GMM_C2'] # [d,d]
            ...
            if alpha == 1:
                ...
                g2 = g2.sum(dim=1, keepdim=True) # [d,1]
            grad = reg_grad + (1/self.s)*torch.cat([g0, g1, g2]) # [2d+1,1]
    ...
    def hess_vect(self, w, v):
        ...
        if self.config['obj_fun'] == 'Autoencoder':
            ...
            z.backward()
            return reg_hess_v + (1/self.s)*torch.cat([T.grad.reshape(-1,1) for T in autoencoder.parameters()]).data
        ...
        if self.config['obj_fun'] == 'GMM':
            ...
            return reg_hess_v + (1/self.s)*torch.cat([H00_v0 + H01_v1 + H02_v2,
        ...
        if self.config['obj_fun'] == 'softmax':
            ...
            return reg_hess_v + (1/self.s)*Hv


    def hess_inv_vect_precond(self, M, v):
        return v
    ...
    def MinresQLP(self, A, b, rtol=None, maxit=None):
        ...
        if self.config['use_preconditioning_in_subproblem'] == False:
            noprecon = True
            pass
        else:
            noprecon = False
            r3 = self.hess_inv_vect_precond(M, r2)
            beta1 = torch.mm(r3.transpose(0,1), r2) #theta
            if beta1 <0:
        ...
        while flag == flag0 and iters < maxit:
            ...
            alfa = torch.mm(r3.transpose(0,1), v) # np.real ?
            r3 = r3 - r2*alfa/beta
            r1 = r2
            r2 = r3
            ...
            if flag != 9:
                rnorm = phi
            stopc = 1 - torch.mm(Ab.transpose(0,1), x)/beta1**2
    ...
    ###############################################################################################
    #---------------------------------- used by GIANT and DINGO ----------------------------------#

    def hess_inv_vect(self, w, b, method='CG', rtol=None, maxit=None):
        '''Compute an approximate least-squares solution to H_i*x=b using CG or MINRES-QLP, 
        where H_i is the local Hessian matrix.
    ...
    def lagrangian_direction(self, weights, g, Hg):
    ...
    def DISCO_P_inv_vect(self, w, b, rtol=None, maxit=None):
        '''Compute an approximate solution to (H_{t,i} + mu*I)*x=b using CG,
        where H_i is the local Hessian matrix.
        ...
        Args:
            ...
            maxit: Maximum iterations.
        '''
        A = lambda v : self.hess_vect(w, v) + self.config['DISCO_mu']*v
        return self.CG(A, b, rtol, maxit)


    ###############################################################################################
    #------------------------------- used by InexactDANE and AIDE --------------------------------#

    def inexactDANE_subproblem(self, weights, gradient, AIDE_tau=0, AIDE_y=0):
        '''Inexactly solve the DANE subproblem using SVRG.
        ...
        while iteration < self.config['subproblem_max_iterations']:
            i = random.randint(0,s-1)
            self.X = X[i:i+1]
            self.Y = Y[i:i+1]
            grad_ell_i_weights = self.grad(weights, AIDE_tau, AIDE_y)
            grad_ell_i_new_weights = self.grad(new_weights, AIDE_tau, AIDE_y)
            step_direction = grad_ell_i_new_weights - grad_ell_i_weights + self.config['inexactDANE_eta']*gradient + self.config['inexactDANE_mu']*(new_weights-weights)
            self.X = X
            self.Y = Y
            new_weights = new_weights - self.config['inexactDANE_SVRG_stepsize'] * step_direction
            iteration += 1
    ...
    ###############################################################################################
    #------------------------------ used by Async-SGD and Sync-SGD -------------------------------#

    def minibatch_grad(self, w):
        '''Return the gradient at point w on a mini-batch of the data.
        ...
        xs = []
        ys = []
        while iteration < self.config['SGD_minibatch_size']:
            i = random.randint(0,s-1)
            xs.append(X[i:i+1])
            ys.append(Y[i:i+1])
            iteration += 1
        ...
        self.X = torch.cat(xs)
        self.Y = torch.cat(ys)
        self.s = self.X.shape[0] # s = self.config['SGD_minibatch_size']
        g = self.grad(w)
